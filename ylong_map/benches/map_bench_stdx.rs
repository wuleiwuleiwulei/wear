...
fn create_capmap_from_iter<T: Hash + Eq + Copy, I: IntoIterator<Item = T>>(
    iter: I,
    cap: usize,
) -> FixedCapMap<T, T, Global> {
    let mut hm = FixedCapMap::new(cap, Global).unwrap();

    for dat in iter {
        hm.insert(dat, dat).unwrap();
    }
...
fn do_bench_new(b: &mut Bencher, src_len: usize) {
    ...
    b.iter(black_box(|| {
        FixedCapMap::<i32, i32, Global>::new(src_len, Global).unwrap()
    }))
}
bench_new!();
...
fn do_bench_insert(b: &mut Bencher, src_len: usize) {
    b.iter(black_box(|| {
        let mut hm = FixedCapMap::new(src_len, Global).unwrap();
        for i in 0..src_len {
...
fn do_bench_remove(b: &mut Bencher, src_len: usize) {
    b.iter(black_box(|| {
        let mut hm = create_capmap_from_iter(0..src_len, src_len);
...
fn do_bench_contains_key(b: &mut Bencher, src_len: usize) {
    let hm = create_capmap_from_iter(0..src_len, src_len);
...
fn do_bench_clone_from(b: &mut Bencher, src_len: usize) {
    let src = create_capmap_from_iter(0..src_len, src_len);

    b.bytes = src_len as u64;

    b.iter(black_box(|| {
        let mut dst = create_capmap_from_iter(0..0, src_len);
